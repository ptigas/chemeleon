...
## Chemeleon for DNG and CSP tasks

While this repository primarily focuses on text-guided generation, we also provide pre-trained models for the `DNG` (De novo generation) and `CSP` (Crystal Structure Prediction) tasks in the [chemeleon-dng](https://github.com/hspark1212/chemeleon-dng.git) repository. These models are trained on the `mp-20` and `alex_mp_20` datasets using a reduced number of diffusion steps (256 timesteps) to enable faster inference.

The architecture (`D3PM`, `DDPM`, and `DSM` for atom types, lattice matrices, and fractional coordinates) is consistent with this repository, except that it excludes the text encoder. For benchmarking purposes, a set of 10,000 structures generated by the DNG model is available in the [benchmarks](https://github.com/hspark1212/chemeleon-dng/tree/main/benchmarks) directory of the chemeleon-dng repository.
